...
<body>
        ...
        <section id="tab-wifi" class="tab-content">
            <div class="card">
                ...
                <div class="section-box">
                    ...
                    <div class="form-row">
                        <label>SSID</label>
                        <input type="text" id="wifiSsid" value="FRT-OS">
                    </div>
                    <div class="form-row">
    ...
    <script>
        ...
        function sendCmd(cmd) {
            fetch('/command?cmd=' + encodeURIComponent(cmd) + '&token=frt12345')
                .then(function(res) {
                    if (!res.ok) {
                        res.text().then(function(t) { showToast('Error: ' + t); });
                    }
                })
                .catch(function(err) {
                    console.error(err);
...
// Token sederhana untuk otorisasi command (sama dengan password WiFi)
static const char* AUTH_TOKEN = "frt12345";

static void handleCommand(AsyncWebServerRequest* request) {
    // Cek token auth dulu — tolak kalau nggak valid
    ...
    if (sscanf(cmd.c_str(), "servo %d %d", &idx, &angle) == 2) {
        ...
        // Ubah invert saja, open/close angle dan posisi sekarang tetap
        servoSetInvert(idx, inv != 0);
        handled = true;
    ...
    // Command yang tidak dikenali → 400 Bad Request
    if (handled) {
